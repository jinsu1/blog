...
-구글계정 로그인 후 위 링크로가서 사본생성 > 확장프로그램 > Apps Script > 8번째 줄 주석 풀고 이메일 입력(이메일 전송 희망시) > 101 번줄
 var row = [Utilities.formatDate(new Date(), 'GMT+9', 'yyyy-MM-dd HH:mm:ss')]; // format date in KST (GMT+9) 한글 시간으로 포맷 > 저장필수 > 우측 상단 배포 > 새배포 > 나에게로 인증, 모든사용자로 설정 후 액세스 승인 > 웹 url 복사 > form action 에 url 추가
```html
...
-title, description을 페이지별로 다르게 설정하고 싶은데 공통 layout.html에서 불러오고 있다면 카페24 어드민 > 쇼핑몰 설정 > 기본 설정 > 검색 엔진 최적화(SEO) > 개별 페이지 SEO 태그에서 설정 가능하다.
